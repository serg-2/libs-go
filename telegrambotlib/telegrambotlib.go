...
func ReplyLowLevel(targetChatId int64, message string, bot *tgbotapi.BotAPI, replyToId int) []int {
	var result []int
	//	log.Printf("Full Message Size: %d\n", len(message))
	for len(message) > MAX_MESSAGE_SIZE {
		// Warning! Beware of splitting runes!
...
// BotInitializeChannelWithCallbacks - initialize new bot for channel with callbacks
func BotInitializeChannelWithCallbacks(config BotConfig) (*tgbotapi.BotAPI, tgbotapi.UpdatesChannel) {
	...
	// Add allowed updates
	// For default behavior
	webHook.AllowedUpdates = []string{"chat_member", "message", "callback_query"}
